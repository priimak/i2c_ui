...
from PySide6.QtGui import Qt, QColor, QMouseEvent, QKeyEvent
from PySide6.QtWidgets import QTableView, QAbstractItemView, QMenu, QWidget
from pytide6 import VBoxPanel, HBoxPanel, PushButton, W, ComboBox, Label

from i2cdgui.app import App
from i2cdgui.i2c_op_thread import ReadRegister, RequestReadAllRegisters, HighlightOff
...
class ResultsTable(QTableView):
    def __init__(self, app: App, reloading_on: Callable[[], None], reloading_off: Callable[[], None]):
        super().__init__(None)
        self.app = app
        ...
        self.context_menu.addAction("Re-read from device", self.re_read_selected_register)
    ...
    def re_read_selected_register(self):
        for index in self.selectedIndexes():
            self.app.re_read_register_at_addr(
                reg_addr=int(self.model.rows[index.row()][0], 16),
                num_bytes=int(len(self.model.rows[index.row()][2]) / 8)
            )
    ...
    def re_read_all_registers(self):
        ...
        for row in self.model.rows:
            self.app.op_thread.commands.put(ReadRegister(
    ...
    def show_register_value(self, data: ShowRegSignalData) -> None:
        ...
        if row == -1:
            # insert new row
            self.model.rows.append([data.register_address, data.hexval, data.binval])
            self.model.rows.sort(key=lambda x: int(x[0], 16))
            row = self.model.indexOfByAddr(data.register_address)
        else:
            self.model.rows[row] = [data.register_address, data.hexval, data.binval]
    ...
    def __init__(self, app: App):
        ...
        self.addWidgets(
            HBoxPanel([
                ...
                ComboBox(items=["once", "every 0.25s", "every 0.5s", "every 1s"], on_text_change=freq_change),
